...
async function createCalendar(b){
  if(process.env.GOOGLE_EMAIL_WEBHOOK_URL&&process.env.GOOGLE_EMAIL_WEBHOOK_SECRET){
    const response=await fetch(process.env.GOOGLE_EMAIL_WEBHOOK_URL,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({
      ...
      title:'Casal Pet Sitter • '+SERVICE[b.service]+' • '+b.tutorName,
      startDate:b.startDate,endDate:b.endDate,
      location:b.street+', '+b.neighborhood+', Viçosa - MG',description:bookingMsg(b)
    })});
    const result=await response.json().catch(()=>({}));
    if(!response.ok||!result.ok)throw new Error('Google Calendar webhook '+response.status+': '+(result.error||'falha ao criar evento'));
    return result.eventId||('google-'+b.id);
  }
